...
func (c *Client) EnsureIndexes(ctx context.Context) error {
	specs := map[string][]mongo.IndexModel{
		domain.CollectionFrictionEvents: {idx("timestamp_start", 1), idx("workflow_stage", 1), idx("friction_layer", 1), idx("friction_type", 1), idx("goal_id", 1), idx("session_id", 1), idx("created_at", 1), idxD(bson.D{{Key: "timestamp_start", Value: -1}, {Key: "workflow_stage", Value: 1}}), idxD(bson.D{{Key: "timestamp_start", Value: -1}, {Key: "friction_layer", Value: 1}}), idxD(bson.D{{Key: "timestamp_start", Value: -1}, {Key: "friction_type", Value: 1}})},
		domain.CollectionWorkSessions:   {idx("started_at", 1), idx("ended_at", 1), idx("created_at", 1)},
